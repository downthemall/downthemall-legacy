...
var DTA_preferences = {
	_pref : null,
	get : function (stringa, predefinito) {
		if (this._pref==null)
			this._pref = Components.classes['@mozilla.org/preferences-service;1'].getService(Components.interfaces.nsIPrefBranch);
	...
	set : function(stringa, value) {
		if (this._pref==null)
			this._pref = Components.classes['@mozilla.org/preferences-service;1'].getService(Components.interfaces.nsIPrefBranch);
			
		if (typeof(value) == "boolean")
			this._pref.setBoolPref(stringa, value);
		else if (typeof(value) == "string")
			this._pref.setCharPref(stringa, value);
		else if (typeof(value) == "number")
			this._pref.setIntPref(stringa, value);
	},
...
var DTA_debug = {
	...
	try {
		...
		var time = new Date();
		var text = String(time.getHours()).formatTimeDate() + ":" + String(time.getMinutes()).formatTimeDate() + ":" + String(time.getSeconds()).formatTimeDate() + ":" + time.getMilliseconds() + "\x0D\x0A\t";
...
var DTA_AddingFunctions = {
	...
	try {
		...
		window.openDialog(
			...
			{ 'url': url, 'description': description, 'refPage': referrer }
		);
	} catch(e) {
