...
function downloadElement(url, dir, num, desc, mask, refPage) {

	dir = dir.addFinalSlash();
	
	// what a mess, keep this backwards compatible :p
	if (url.url) {
		if (url.usable.length)
		{
			var name = url.usable.getUsableFileName();
		} else {
			var name = url.url.getUsableFileName();
			try {
				name = decodeURIComponent(name);
			} catch (ex) {
				Debug.dump('downloadElement ctor: empty usableURL; decoding url failed', ex);
			}
		}
		this.url = url.url;
		this.usableURL = url.usable;
	} else {
		var name = String(url).getUsableFileName();
		this.url = url;
		this.usableURL = '';
		try {
			name = decodeURIComponent(name);
	...
	this.destinationName = this.fileName = name;
	this.mask = mask;
	this.numIstance = num;
...
downloadElement.prototype = {
	...
	reDownload : function() {
		// replace names
		if (this.usableURL.length)
		{
			var name = this.usableURL.getUsableFileName();
		}
		else
		{
			var name = this.url.getUsableFileName();
			try {
				name = decodeURIComponent(name);
			} catch(ex) {
				Debug.dump('donwloadElement::reDownload: empty usableURL; decoding url failed', ex);
			}
		}
		this.destinationName = this.fileName = name;
	...
	getHeader : function() {
		Debug.dump(this.url + " (" + this.refPage.spec +"): getHeader()");
		Prefs.refresh();
	...
	buildFromMask : function(dir, mask) {try {
		...
		uri.spec = this.usableURL.length ? this.usableURL : this.url;
		...
		if (uripath.length) {
			...
			if (!this.usableURL.length) {
				try {
					...
					Debug.dump('donwloadElement::buildFromMask: empty usableURL; decoding url failed', ex);
				}
			}
		}
	...
	checkFilenameConflict : function() {try {
		...
		var shortUrl = (this.usableURL.length?this.usableURL:this.url).cropCenter(maxUrlSize);
		var s = -1;
		if (!this.isCompleted && this.isRunning) {
			if (realDest.exists()) {
				...
				if (p != -1) {
					s = askForRenaming(
						strbundle.getString("samedestination", [shortUrl, this.destinationName, inProgressList[p].d.url]) + " " + strbundle.getString("whatdoyou"),
...
HTTPResponseHeaderVisitor.prototype = {
	...
	visitHeader : function(aHeader, aValue) {try {
		...
		if ((header == 'content-type' || header == 'content-disposition') && this.fileName == null) {
			...
			if (value) this.fileName = value[3].getUsableFileName();
		}
	} catch (ex) {
			...
			if (!(chan instanceof Components.interfaces.nsIHttpChannel)) {
				...
				if (this.isHeaderHack) {
					...
					if (!d.alreadyMaskedName) {
						...
						if (aRequest.URI.spec != d.url) {
							d.fileName = aRequest.URI.spec.getUsableFileName();
							try {
								d.fileName = decodeURIComponent(d.fileName);
							} catch (ex) {
								Debug.dump('onProgressChange: decodeURIComponent failed for redirected header!', ex);
							}
						}
					...
					if (d.isResumable && d.totalSize > 2 * Prefs.minChunkSize && d.maxChunks > 1) {
						// in caso di redirect ci salviamo il reale url
						d.url = aRequest.URI.spec;
...
function populateListbox(d) {
	...
	itemNode.setAttribute("value", d.url);
	var id = d.url + (new Date()).getTime() + String(Math.floor(10000 * (Math.random() % 1)));
	...
	if (Prefs.showOnlyFilenames)
		...
		nomefile.setAttribute("label", " " + d.url);
...
function getInfo() {
	...
	if (t.length > 0) window.openDialog("chrome://dta/content/dta/info.xul","_blank","chrome, centerscreen, dialog=no", t, this);
}
	...
	load: function() {try {
		...
		for (var k=0; k<list.length; k++) {try {
			...
			var d = new downloadElement(
				{ url: this.get(down, "url"), usable: this.get(down, 'usableURL') },
...
try {
		...
		var n = row.value;
		$("infoURL").value = downloadList[n].url;
